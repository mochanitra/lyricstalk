    ...
    <form class="container" @submit.prevent="submit">
      ...
      <div class="row">
        <div class="col-12 _dp-f _fdrt-cl _alit-ct _pdt-32px _pdbt-64px">
          ...
          <button type="submit" class="bio-button">Share Quiz (URL)</button>
        </div>
      </div>
    </form>
  </div>
    ...
    async submit() {
      ...
      this.$store.commit("SET_MYQUIZ_PROFILE", {
        ...
        owner: {
          ...
          photoURL: this.$store.state.auth.photoURL,
          displayName: this.$store.state.auth.displayName
        }
      });
      // const res = await this.$axios.$post(
      //   `https://lyricstalk-1fb09.firebaseio.com/quizes.json`,
      ...
      updates[
        "/user/" + this.$store.state.auth.uid + "/quizesList/" + newQuizKey
      ] = this.$store.state.myQuiz;
